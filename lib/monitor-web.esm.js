...
const nextTime =
 global$7.requestIdleCallback ||
 global$7.requestAnimationFrame ||
 ((callback) => global$7.setTimeout(callback, 17));
...
function report(url, data = {}, immediate = false) {
  ...
  if (caches.length >= max) {
    ...
    timer$1 = global$6.setTimeout(send, 5000);
  }
}
...
function proxyDomClick(handler) { 
  ...
  addInstrumentationHandler(instrumentType$1.DC, handler);

  if (getInstrumented(instrumentType$1.DC).length > 1) {
    return
  }
  ...
  ['mousedown', 'touchstart'].forEach(eventType => {
    global.document.addEventListener(eventType, function(e) {
      ...
      timer = global.setTimeout(() => {
        ...
        if (!target) return
        triggerHandlers(instrumentType$1.DC, { eventType, target, e });
      }, 500);
    });
  });
}
...
const dispatchEvent = function(type) {
  ...
  global$5.dispatchEvent(e);
};

function wrHistory() {
  if ('history' in global$5) {
    ['pushState', 'replaceState'].forEach(type => {
      fill(global$5.history, type, function(original) {
        return function() {
...
function proxyHistory(handler) {
  global$5.addEventListener('replaceState', (e) => handler(e), true);
  global$5.addEventListener('pushState', (e) => handler(e), true);
}

// hash 变化除了触发 hashchange ,
// 也会触发 popstate 事件,而且会先触发 popstate 事件，我们可以统一监听 popstate
// 这里可以考虑是否需要监听 hashchange,或者只监听 hashchange
function proxyHash(handler) {
  global$5.addEventListener('hashchange', (e) => handler(e), true);
  global$5.addEventListener('popstate', (e) => handler(e), true);
}
...
const getOriginInfo = () => {
  return {
    referrer: global$2.document.referrer,
    type: global$2.performance?.navigation.type || '',
  }
};
...
function initRouterChange(websdk) {
...
function initClickRecord(websdk) {
...
function initHttpRecord(websdk) {
...
function initBreadcrumb(websdk) {
  init(() => {
...
export { WebSDK as default };
//# sourceMappingURL=monitor-web.esm.js.map
